...
raw_data{1} = cell(1, num_line); % 存储每条扫描线的多阵元原始数据
raw_data{2} = cell(1, num_line);
traw_data = cell(1, num_line);
...
ele_pos{1} = probe.ele_pos(1:32, :);
ele_pos{2} = probe.ele_pos(33:end, :);
...
nz = grid_s(2);
das = zeros(nx, nz);
foc = zeros(rx_num_line/2, nz);
tfoc = zeros(rx_num_line, nz);

thamming = hamming(element_num);
hamming_win{1} = thamming(1:32);
hamming_win{2} = thamming(33:64);
...
for i = 1:rx_num_line
    ...
    tbeamdata = zeros(2, nz);
    for a = 1 : 2
        data = raw_data{a}{i}';
        txdel = sqrt(sum((squeeze(grid(i, :, :)) - squeeze(tx_ori(i, :, :))).^2, 2))';   % (1026x3) - (1x3)
        rxdel = sqrt(sum((reshape(grid(i, :, :), [], 1, 3) - reshape(ele_pos{a}, [1, size(ele_pos{a})])).^2, 3))';   % (1026x1x3) - (1x32x3)
        delays = ((txdel + rxdel) / c - tstart(i)) * fs;

        xc = 1 : size(data, 2);
        for j = 1 : element_num/2
            foc(j, :) = interp1(xc, data(j, :), delays(j, :), 'linear', 0.0);
        end

        apods = apod_focus(grid(i, :, :), ele_pos{a}, 1, hamming_win{a});
        foc = foc .* apods;

        tbeamdata(a, :) = sum(foc);
    end

    beamdata = sum(tbeamdata, 1);
